import React, { useEffect, useState } from "react";
import HexagonImage from "./HexagonImage/HexagonImage";
import { useAuth } from "../context/AuthContext";
import { useNavigate } from "react-router-dom";
import Tooltip from "./Tooltip";
import Profile from "./Profile";

const imageUrls = [
    "data:image/jpeg;base64,",
    "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR3NdS5tXnPRWtZbLGtuagKWphUYHHbq3K4pw&usqp=CAU",
    "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSRwkCYt_O1nSklYssElNwJ3q6gfU02iUPFxg&usqp=CAU",
    "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRForYSfhEDE0TB5ugx734-uuHSdzoQU1MFLg&usqp=CAU",
    "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS71HOoAeaPsLFtpcW8tka5P22fJnd17NvePQ&usqp=CAU",
    "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRstL3ZNZRxIqh0TBG5UsHJKjEV-YA92A3p9w&usqp=CAU",
];

function getRandomImageUrl() {
    const randomIndex = Math.floor(Math.random() * imageUrls.length);
    return imageUrls[randomIndex];
}

export default function Leaderboard({ LeaderboardItems, handleUserRowClick }) {
    const { user } = useAuth();

    const [me, setMe] = useState(null);

    useEffect(() => {
        if (user && LeaderboardItems) {
            const me = LeaderboardItems.find((_user) => _user.userId == user.userId);
            setMe(me);
        }
    }, [user, LeaderboardItems]);

    useEffect(() => {
        LeaderboardItems.forEach((item) => {
            item.img = getRandomImageUrl();
        });
        console.log(LeaderboardItems);
    }, [LeaderboardItems]);

    return (
        <div className="scroll-hidden overflow-y-scroll grow">
            <table className="w-full h-fit max-h-full text-base text-center align-middle">
                <tbody>
                    <tr className="bg-white1 w-full text-black2 sticky top-0 z-30 h-[28px]">
                        <th className="p-0 pb-1 w-3/12 font-normal ">Rank</th>
                        <th className="p-0 pb-1  font-normal text-start items-center ">User</th>
                        <th className="p-0 pb-1 w-3/12 font-normal ">Score</th>
                    </tr>
                    <tr>
                        <td className="bg-white3 h-[1px] sticky top-[28px]" colSpan={3}></td>
                    </tr>

                    {me ? (
                        <>
                            <tr className="bg-white1 w-full font-medium sticky top-[30px] h-[50px] z-30">
                                <td className="p-0 h-[50px] w-3/12">{`#${me.rank}`}</td>
                                <td className="p-0 h-[50px] flex text-start items-center justify-start gap-2">
                                    <HexagonImage profileImg={me.img} resize={1 / 2} />
                                    <p className="truncate ">{me.name}</p>
                                </td>
                                <td className="p-0 h-[50px] w-3/12">{me.score}</td>
                            </tr>

                            <tr>
                                <td className="bg-white3 h-[1px] z-30 sticky top-[80px]" colSpan={3}></td>
                            </tr>
                            <tr>
                                <td className="bg-white1 h-[10px] z-30 sticky top-[82px]" colSpan={3}></td>
                            </tr>
                        </>
                    ) : (
                        <tr>
                            <td className="bg-white1 h-[10px] z-30 sticky top-[30px]" colSpan={3}></td>
                        </tr>
                    )}

                    {LeaderboardItems &&
                        LeaderboardItems.map((user) => (
                            <tr
                                id={`row-${user.userId}`}
                                key={user.userId}
                                data-tooltip-id={`user-${user.userId}-tooltip`}
                                className="font-medium w-full cursor-pointer hover:scale-105 transition-all duration-150"
                                onClick={() => handleUserRowClick(user.userId)}
                            >
                                <td
                                    className={` mt-sm
                              p-0 py-2 w-3/12
                              ${user.rank == 1 ? "text-bgRed1" : user.rank == 2 ? "text-bgGreen1" : user.rank == 3 ? "text-bgBlue1" : "text-black1"}
                              `}
                                >
                                    {`#${user.rank}`}
                                </td>
                                <td className="p-0 py-2 h-full flex text-start items-center justify-start gap-2 ">
                                    <HexagonImage profileImg={user.img} resize={1 / 2} />
                                    <p className="truncate ">{user.name}</p> {/*TODO: Change to username NOT firstLast */}
                                </td>
                                <td className="p-0 py-2 only:w-3/12">{user.score}</td>
                            </tr>
                        ))}
                </tbody>
            </table>
            {LeaderboardItems &&
                LeaderboardItems.map((user) => (
                    <React.Fragment key={user.userId}>
                        <Tooltip
                            id={`user-${user.userId}`}
                            place="bottom"
                            tooltip={
                                <div className="w-[320px]">
                                    <Profile
                                        profile={{
                                            id: user.userId,
                                            name: user.name,
                                            // TODO: Modify API to return title, level, streak
                                            title: "Newcomer",
                                            level: 1,
                                            streakDays: null,
                                            profileImg: null,
                                        }}
                                    />
                                </div>
                            }
                        />
                    </React.Fragment>
                ))}
        </div>
    );
}
